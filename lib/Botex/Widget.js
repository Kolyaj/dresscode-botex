Botex.Widget = Bricks.inherit(Botex.Tag, {
    ...
    _createElement: function(meta) {
        ...
        var className = meta.className.map(function(cls) {
            if (cls instanceof Botex.Mutable) {
                this._domEventsController.on(cls, 'beforechange', this.$$_onMutableClassNameChange, {
                    el: el,
                    that: this
                });
                return cls.getValue();
    ...
    $$_onMutableClassNameChange: function(evt) {
        ...
        var prevClassNames = evt.value.split(/\s+/).filter(Boolean);
        var currClassNames = evt.nextValue.split(/\s+/).filter(Boolean);
